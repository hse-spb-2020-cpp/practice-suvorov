...
int main() {
    ...
    // Перед передачей владения берём себе ещё одно владение
    // при помощи json_object_get(). Как копирование shared_ptr<>.
    json_object_array_add(result, json_object_get(my_name));
